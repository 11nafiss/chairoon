...
exports.updateMe = catchAsync(async (req, res, next) => {
  ...
  // Ensure to use options to return the new document and run validators
  const userDoc = await User.findByIdAndUpdate(req.user._id, updateData, {
    new: true,
    runValidators: true,
  });
...
exports.getSuggestedUser = catchAsync(async (req, res, next) => {
...
exports.getFriends = catchAsync(async (req, res, next) => {
  const this_user = await User.findById(req.user._id).populate("friends", "-password");
  res.status(200).json({
    status: "success",
...
exports.deleteUser = catchAsync(async (req, res, next) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(id)) {
    next(createHttpError(400, "Invalid User ID format"));
  }
